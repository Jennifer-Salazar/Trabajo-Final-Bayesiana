...
```{r, echo=FALSE}
# años <- sample(seq(2003, 2022), 1000, replace = T)
# error <- rnorm(1000, mean=0, sd=sqrt(mean(varianza)))
# precio <- media[1]+media[2]*años + error
```
...
library(MASS)
library(invgamma)
normal <- mvrnorm(1000,media,precision)
gamma_inversa <- rinvgamma(1000, alfa, rate = beta)
...
```{r fig.align="center", echo=FALSE}
hist(apriori, xlab='apriori', ylab='Densidad', main="Apriori", col="cyan3", freq = FALSE)
lines(density(apriori), lwd=1.5)
grid()
```


```{r, echo=FALSE}
# betas <- df_parametros[,-3]
# varianza <- df_parametros[,3]
# (((1/varianza)^(alfa+1.5))*exp(-(1/df_parametros[,3])*(alfa+(1/2)*t(betas-media)*var*(betas-media))))
```
...
Modelo de regresión asociado a la muestra:
...
library(latex2exp)
plot(muestra$Año, muestra$Precio/1000000, xlab="Año", ylab="Precio en millones de pesos", pch=19, col="cyan4", main="Modelo de regresión lineal asociado a la muestra")
abline(coef(modelo_muestra)/1000000, col="purple")
legend('topleft', legend="Precio = -9222.394667 + 4.600667xAño")
grid()
```
