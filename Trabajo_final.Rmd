...
```{r, echo = FALSE}
require(kableExtra)
kable(modelo, col.names = c("β₀", "β₁", "σ²"), caption = "Parametros de modelo de regresión lineal simple obtenidos via MCMC") %>%
  kable_paper() %>%
  row_spec(0, bold = T, color = "white", background = "#039794")  %>%
  scroll_box(width = "100%", height = "200px")
```
...
```{r fig.align="center", echo=FALSE}
hist(modelo[,3], xlab='sigma_2', ylab='Densidad', main=expression(paste("Distribucion de ", sigma^2, " estimado")), col="cyan3", freq = FALSE)
...
**Ejemplo:** Se espera que un carro que sea del año 2015 tenga un precio de aproximadamente 43 millones de pesos colombianos $Precio = -8088.042632+ 4.035309 \times 2015 = 43.105 \approx 43~millones$
...
$$c1=\hat{\alpha}_0 = `r c1`$$

Parámetro de escala

$$d1=\hat{\gamma}_0 = `r d1`$$


<br>

**Comparación de la distribución apriori con la posterior**

```{r, echo=FALSE}
parametros <- data.frame(beta_0 = c(media[1], b1[1]), beta_1=c(media[2], b1[2]), media_var = c(m, m.var1), varianza_var=c(v, v.var1), alfa = c(alfa, c1), beta=c(beta, d1), row.names = c("Apriori", "Posteriori"))
kable(parametros, col.names = c("β₀", "β₁", "E(varianza)",  "Var(varianza)", "c", "d"), caption = "Parametros de modelo de regresión lineal Mediante la distribución apriori y la distribución posteriori", row.names = T) %>%
  kable_paper() %>%
  row_spec(0, bold = T, color = "white", background = "#039794")  %>%
  scroll_box(width = "100%", height = "200px")
```
...
Conclusión: En base al resultado del promedio del $\beta_0$ y $\beta_1$ que es la tasa de cambio del precio de oferta de los mazda 3 dado el año  hallado a partir de la distribución dada por el experto (distribución apriori) y a partir de la obtenida mediante la muestra y la distribución apriori (distribución posteriori) se ve la gran similaridad entre estas, lo que indica que la estimación de los precios de oferta dados por el experto son demasiado similares a los extraidas en la muestra de internet, lo que indica que el conocimiento del experto da confiabilidad; realmente el cambio se ve en es la varianza que se estima en los modelos de regresión mediante la información dada por el experto y la extraida de internet, lo que indica que la dada por el experto es información más precisa (varianza más pequeña) que la extraida de internet.
